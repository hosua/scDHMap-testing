...
class scDHMap(nn.Module):
    ...
    ###################################################################
    def save_latent(self, x, epoch_itr, root_name):
        epoch_itr = str(epoch_itr).zfill(4)
        latent_fname = f"{data_dir}/{root_name}_{epoch_itr}.txt"
        curr_latent = self.encodeBatch(torch.tensor(x).double().to("cuda"))
        np.savetxt(latent_fname, curr_latent, delimiter=",")
    ###################################################################
    ...
    def pretrain_autoencoder(self, X, X_raw, size_factor, lr=0.001, pretrain_iter=400, ae_save=True, ae_weights="AE_weights.pth.tar"):
        ...
        for epoch in range(pretrain_iter):
            ...
            self.save_latent(X, itr, "pre_latent")
            itr += 1
...
#                dist_X_pca_batch = dist_X_pca[batch_indices][:, batch_indices]
            ...
            ###################################################################
            self.save_latent(X, epoch_itr, "latent")
            epoch_itr += 1
            ###################################################################
